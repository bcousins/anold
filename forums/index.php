...
//Query the DB for Categories ($sqlcat), forums ($sqlfrm), Recent Posts - And much, much, much more.
$sqlcat1 = mysql_query('SELECT * FROM  `f-cat` WHERE `id` = 6') or die(mysql_error());
$sqlforum1 = mysql_query('SELECT * FROM  `f-forums` WHERE `cat_id` = 6 ORDER BY `permission`') or die(mysql_error());
//Prepare the div (??)
while($row = mysql_fetch_array($sqlcat1))
  {
	echo '<div class="cat">';
